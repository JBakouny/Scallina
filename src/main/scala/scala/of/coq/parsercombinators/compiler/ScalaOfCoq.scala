...
class ScalaOfCoq(coqTrees: List[Sentence], curryingStrategy: CurryingStrategy) {
  ...
  private def coqTypeToTreeHuggerType(typeTerm: Term): Type = typeTerm match {
    ...
    case ExplicitQualidApplication(id, genericTypeParams) ⇒
      createTypeWithGenericParams(coqTypeToTreeHuggerType(id), genericTypeParams.map(coqTypeToTreeHuggerType))
  ...
  private def convertQualitToPrimitiveValue(inputValue: String) =
    inputValue match {
      ...
      case anythingElse ⇒
        throw new IllegalStateException("This qualid is not a primitive value in Scala: " + anythingElse)
    }
    ...
    val caseClassHierarchy: List[Tree] =
      indBodyItems flatMap {
        ...
        case InductiveBodyItem(Ident(name), binders, indBodyItemType) ⇒
          require(indBodyItemType.isEmpty, "Return types of inductive body items should be ommitted")
          binders.fold {
            /*
  ...
  private def convertTuple[A, B <: Tree](tupleElems: List[A], converterFunction: A ⇒ B): Tree = {
    tupleElems match {
      case Nil ⇒
        throw new IllegalStateException("Cannot convert a tuple with no elements!")
      case p :: Nil ⇒
  ...
  private object PatternUtils {
    ...
    private def convertMatchItem(matchItem: MatchItem): Tree = matchItem match {
      ...
      case anythingElse ⇒
        throw new IllegalStateException("The following Coq match item is not supported: " + anythingElse.toCoqCode)
    }
  ...
  object RecordUtils {
    ...
    def convertRecordInstance(
        ...
        args: Option[Binders],
        recordType: String,
        concreteRecordFields: List[ConcreteRecordField]
    ): Tree = {
      ...
      (definedBinders, implementedBinders) match {
        ...
        case (Some(Binders(defBinders)), Some(Binders(implBinders))) ⇒
          ...
          Some(Binders(expandedDefBinders.zip(expandedImplBinders).map {
            ...
            case (ExplicitBinderWithType(List(_), _), implBinder @ ExplicitBinderWithType(List(_), _)) ⇒
              implBinder
    ...
    private def createRecordConstructionFunction(record: Record): Option[Tree] = {
      ...
      constructor.map { constructorName ⇒
        ...
        val recordType =
          createTypeWithGenericParams(recordName, typeBinderParams.map(typeVar ⇒ TYPE_REF(typeVar.name)))
    ...
    private def assignFieldAliasesToNewRecordAbstractField(
      ...
      (abstractFields zip abstractFields.map(recordFieldIsTypeField)) map {
        case (typefield, true) ⇒
          convertAbstractRecordField(typefield) := TYPE_REF(namePrefix + extractNameFromRecordField(typefield))
        case (valuefield, false) ⇒
          convertAbstractRecordField(valuefield) := REF(namePrefix + extractNameFromRecordField(valuefield))
      }
    ...
    private def computeFunctionReturnType(binders: Option[Binders], typeTerm: Term): Type =
      binders match {
        case Some(params) ⇒
          convertBindersToFunctionReturnType(params) TYPE_=> coqTypeToTreeHuggerType(typeTerm)

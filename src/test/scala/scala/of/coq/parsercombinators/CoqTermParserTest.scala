...
class CoqTermParserTest extends FunSuite {
  ...
  test("""Testing "forall a, (a -> a)" """) {
    CoqTermParser("forall a, (a -> a)") should parse(
      ForAll(Binders(List(ExplicitSimpleBinder(Name(Some(Ident("a")))))),
             BetweenParenthesis(
  ...
  test("""Testing "forall a b c, (a -> b) -> (b -> c) -> (a -> c)" """) {
    CoqTermParser("forall a b c, (a -> b) -> (b -> c) -> (a -> c)") should parse(
      ForAll(
        Binders(
          List(ExplicitSimpleBinder(Name(Some(Ident("a")))),
               ExplicitSimpleBinder(Name(Some(Ident("b")))),
               ExplicitSimpleBinder(Name(Some(Ident("c")))))),
        Term_->(
          BetweenParenthesis(
            Term_->(Qualid(List(Ident("a"))), Qualid(List(Ident("b"))))),
          Term_->(
            BetweenParenthesis(
              Term_->(Qualid(List(Ident("b"))), Qualid(List(Ident("c"))))),
            BetweenParenthesis(
              Term_->(Qualid(List(Ident("a"))), Qualid(List(Ident("c")))))
          )
        )
      ))
  }
  ...
  test("""Testing "fun s => 5" """) {
    CoqTermParser("fun s => 5") should parse(
      Fun(Binders(List(ExplicitSimpleBinder(Name(Some(Ident("s")))))),
          Number(5)))
  }

  test("""Testing "fun (x : nat) => x" """) {
    CoqTermParser("fun (x : nat) => x") should parse(
      Fun(Binders(List(ExplicitBinderWithType(List(Name(Some(Ident("x")))),
      ...
      Fun(
        Binders(
          List(
            ...
            ExplicitBinderWithType(List(Name(Some(Ident("l"))),
                                        Name(Some(Ident("r")))),
                                   Qualid(List(Ident("A"))))
          )),
        Qualid(List(Ident("l")))
  ...
  test("""Testing "fun {A B} (l: A) (r: B) => r" """) {
    CoqTermParser("fun {A B} (l: A) (r: B) => r") should parse(
      Fun(
        Binders(
          List(
            ImplicitBinder(List(Name(Some(Ident("A"))), Name(Some(Ident("B")))),
                           None),
            ExplicitBinderWithType(List(Name(Some(Ident("l")))),
                                   Qualid(List(Ident("A")))),
            ExplicitBinderWithType(List(Name(Some(Ident("r")))),
                                   Qualid(List(Ident("B"))))
          )),
        Qualid(List(Ident("r")))
      )
  ...
  test("""Testing "fun (a b c : nat) => b" """) {
    CoqTermParser("fun (a b c : nat) => b") should parse(
      Fun(
        Binders(
          List(
            ExplicitBinderWithType(List(Name(Some(Ident("a"))),
                                        Name(Some(Ident("b"))),
                                        Name(Some(Ident("c")))),
                                   Qualid(List(Ident("nat")))))),
        Qualid(List(Ident("b")))
  ...
  test("""Testing "fun (_ b _ : nat) => b" """) {
    CoqTermParser("fun (_ b _ : nat) => b") should parse(
      Fun(Binders(
            List(ExplicitBinderWithType(
              List(Name(None), Name(Some(Ident("b"))), Name(None)),
              Qualid(List(Ident("nat")))))),
          Qualid(List(Ident("b")))))
  }
  ...
  test("""Testing "fun s _ a => 5" """) {
    CoqTermParser("fun s _ a => 5") should parse(
      Fun(Binders(
            List(ExplicitSimpleBinder(Name(Some(Ident("s")))),
                 ExplicitSimpleBinder(Name(None)),
    ...
    CoqTermParser("""
      ...
      LetFixIn(
        FixBody(
          Ident("rightMost"),
          Binders(
            List(ImplicitBinder(List(Name(Some(Ident("A")))), Some(Type)),
                 ExplicitBinderWithType(List(Name(Some(Ident("t")))),
                                        Qualid(List(Ident("BinTree")))))),
          None,
          ...
          Match(
            ...
            List(
              PatternEquation(
                List(
                  MultPattern(List(ConstructorPattern(
                    Qualid(List(Ident("L"))),
                    List(QualidPattern(Qualid(List(Ident("x"))))))))),
                Qualid(List(Ident("x")))),
              PatternEquation(
                List(
                  MultPattern(
                    List(ConstructorPattern(
                      Qualid(List(Ident("N"))),
                      List(QualidPattern(Qualid(List(Ident("l")))),
        ...
        UncurriedTermApplication(
          ...
          List(
            Argument(
              None,
              BetweenParenthesis(UncurriedTermApplication(
                Qualid(List(Ident("N"))),
                List(
                  Argument(None,
                           BetweenParenthesis(UncurriedTermApplication(
                             Qualid(List(Ident("L"))),
                             List(Argument(None, Number(3)))))),
                  Argument(
                    None,
                    BetweenParenthesis(UncurriedTermApplication(
                      Qualid(List(Ident("N"))),
                      List(
                        Argument(None,
                                 BetweenParenthesis(UncurriedTermApplication(
                                   Qualid(List(Ident("L"))),
                                   List(Argument(None, Number(5)))))),
                        Argument(None,
                                 BetweenParenthesis(UncurriedTermApplication(
                                   Qualid(List(Ident("L"))),
                                   List(Argument(None, Number(7))))))
                      )
                    ))
                  )
                )
  ...
  test("""Testing "let x := 3 in fun y => y % x" """) {
    CoqTermParser("let x := 3 in fun y => y % x") should parse(
      SimpleLetIn(
        ...
        None,
        Number(3),
        Fun(Binders(List(ExplicitSimpleBinder(Name(Some(Ident("y")))))),
            Term_%(Qualid(List(Ident("y"))), Ident("x")))))
  }
  ...
  test("""Testing "let sumAlias (a b : nat) := sum a b in sumAlias 3 7" """) {
    CoqTermParser("let sumAlias (a b : nat) := sum a b in sumAlias 3 7") should parse(
      SimpleLetIn(
        Ident("sumAlias"),
        Some(
          Binders(List(ExplicitBinderWithType(List(Name(Some(Ident("a"))),
                                                   Name(Some(Ident("b")))),
                                              Qualid(List(Ident("nat"))))))),
        None,
        UncurriedTermApplication(Qualid(List(Ident("sum"))),
                                 List(Argument(None, Qualid(List(Ident("a")))),
                                      Argument(None,
                                               Qualid(List(Ident("b")))))),
        UncurriedTermApplication(Qualid(List(Ident("sumAlias"))),
                                 List(Argument(None, Number(3)),
  ...
  test("""Testing "let fix right (l r : nat) : nat := r in right 7 3" """) {
    CoqTermParser("let fix right (l r : nat) : nat := r in right 7 3") should parse(
      LetFixIn(
        ...
        UncurriedTermApplication(Qualid(List(Ident("right"))),
                                 List(Argument(None, Number(7)),
  ...
  test(
    """Testing "let fix right (l r : nat) { struct l } := r in right 7 3" """) {
      ...
      LetConstructorArgsIn(
        ...
        UncurriedTermApplication(Qualid(List(Ident("sum"))),
                                 List(Argument(None, Qualid(List(Ident("l")))),
                                      Argument(None, Qualid(List(Ident("r"))))))
      ))
  }
  ...
  test("""Testing "let (l, r) as p return nat := Node 3 7 in sum l r" """) {
    CoqTermParser("let (l, r) as p return nat := Node 3 7 in sum l r") should parse(
      LetConstructorArgsIn(
        List(Name(Some(Ident("l"))), Name(Some(Ident("r")))),
        Some(DepRetType(Some(Name(Some(Ident("p")))),
                        ReturnType(Qualid(List(Ident("nat")))))),
        UncurriedTermApplication(Qualid(List(Ident("Node"))),
                                 List(Argument(None, Number(3)),
                                      Argument(None, Number(7)))),
        UncurriedTermApplication(Qualid(List(Ident("sum"))),
  ...
  test("""Testing "let ' pair l r := pair 3 7 return nat in sum l r" """) {
    CoqTermParser("let ' pair l r := pair 3 7 return nat in sum l r") should parse(
      LetPatternIn(
        ConstructorPattern(Qualid(List(Ident("pair"))),
                           List(QualidPattern(Qualid(List(Ident("l")))),
                                QualidPattern(Qualid(List(Ident("r")))))),
  ...
  test("""Testing "if a then 5 else 7" """) {
    CoqTermParser("if a then 5 else 7") should parse(
      TermIf(Qualid(List(Ident("a"))), None, Number(5), Number(7)))
  }
  ...
  test("""Testing "if a as b return nat then 5 else 7" """) {
    CoqTermParser("if a as b return nat then 5 else 7") should parse(
      TermIf(Qualid(List(Ident("a"))),
             Some(DepRetType(Some(Name(Some(Ident("b")))),
                             ReturnType(Qualid(List(Ident("nat")))))),
             Number(5),
             Number(7)))
  }
  ...
  test("""Testing "2 : a" """) {
    CoqTermParser("2 : a") should parse(
      Term_:(Number(2), Qualid(List(Ident("a")))))
  }
  ...
  test("""Testing "a -> a" """) {
    CoqTermParser("a -> a") should parse(
      Term_->(Qualid(List(Ident("a"))), Qualid(List(Ident("a")))))
  }
  ...
  test("""Testing "sum (x := 7) 3" """) {
    CoqTermParser("sum (x := 7) 3") should parse(
      UncurriedTermApplication(Qualid(List(Ident("sum"))),
                               List(Argument(Some(Ident("x")), Number(7)),
                                    Argument(None, Number(3)))))
  }
      ...
      UncurriedTermApplication(
        ...
        List(
          Argument(
            None,
            BetweenParenthesis(
              UncurriedTermApplication(Qualid(List(Ident("S"))),
  ...
  test("""Testing "@Tree A" """) {
    CoqTermParser("@Tree A") should parse(
      ExplicitQualidApplication(Qualid(List(Ident("Tree"))),
  ...
  test("""Testing "3 + 7" """) {
    CoqTermParser("3 + 7") should parse(
      InfixOperator(Number(3), "+", Number(7)))
  }

  test("""Testing "7 - 3 - 2" """) {
    CoqTermParser("7 - 3 - 2") should parse(
      InfixOperator(Number(7), "-", InfixOperator(Number(3), "-", Number(2))))
  }

  test("""Testing "3 * 7 * 3" """) {
    CoqTermParser("3 * 7 * 3") should parse(
      InfixOperator(Number(3), "*", InfixOperator(Number(7), "*", Number(3))))
  }
  ...
  test("""Testing "x :: xs" """) {
    CoqTermParser("x :: xs") should parse(
      InfixOperator(Qualid(List(Ident("x"))), "::", Qualid(List(Ident("xs")))))
  }
              ...
              List(
                Argument(
                  ...
                  BetweenParenthesis(
                    UncurriedTermApplication(
                      Qualid(List(Ident("Node"))),
                      List(Argument(None, Qualid(List(Ident("Leaf")))),
    ...
    """) should parse(
      Match(
        List(MatchItem(
          Qualid(List(Ident("H"))),
          None,
          Some(MatchItemPattern(Qualid(List(Ident("eq"))),
                                List(UnderscorePattern,
                                     UnderscorePattern,
                                     QualidPattern(Qualid(List(Ident("z")))))))
        )),
        Some(ReturnType(UncurriedTermApplication(
          Qualid(List(Ident("eq"))),
          List(Argument(None, Qualid(List(Ident("A")))),
               Argument(None, Qualid(List(Ident("z")))),
               Argument(None, Qualid(List(Ident("x")))))
        ))),
        List(PatternEquation(
          List(MultPattern(List(
            ConstructorPattern(Qualid(List(Ident("eq_refl"))),
  ...
  test("""Testing "record.(field)" """) {
    CoqTermParser("record.(field)") should parse(
      SimpleProjection(Qualid(List(Ident("record"))),
  ...
  test("""Testing "record.(field 3 7)" """) {
    CoqTermParser("record.(field 3 7)") should parse(
      ApplicationProjection(
        ...
        UncurriedTermApplication(Qualid(List(Ident("field"))),
                                 List(Argument(None, Number(3)),
                                      Argument(None, Number(7))))))
  }
  ...
  test("""Testing "record.(@ field nat 7)" """) {
    CoqTermParser("record.(@ field nat 7)") should parse(
      ExplicitApplicationProjection(
        Qualid(List(Ident("record"))),
        ExplicitQualidApplication(Qualid(List(Ident("field"))),
                                  List(Qualid(List(Ident("nat"))), Number(7)))))
  }
  ...
  test("""Testing "anIdentifier" """) {
    CoqTermParser("anIdentifier") should parse(
      Qualid(List(Ident("anIdentifier"))))
  }
